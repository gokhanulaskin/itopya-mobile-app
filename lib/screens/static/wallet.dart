...
import 'package:itopya_mobile/core/storage.dart';
import 'package:itopya_mobile/model/credit_card.dart';
import 'package:itopya_mobile/widgets/bottomNavigation.dart';
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  showAddCard() {
    showModalBottomSheet(
      context: context,
      isScrollControlled:
          true, // Klavye açıkken ekranın yukarı kaymasını sağlar
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setState) {
            // StatefulBuilder, modal alt sayfadaki değişiklikleri güncellemek için gereklidir
            return SingleChildScrollView(
              // Küçük ekranlarda kaydırmayı sağlar
              padding: EdgeInsets.only(
                bottom: MediaQuery.of(context)
                    .viewInsets
                    .bottom, // Klavyenin açık olması durumunda alt kısmın görünmesini sağlar
              ),
              child: Container(
                ...
                child: Column(
                  ...
                  mainAxisSize:
                      MainAxisSize.min, // Minimum boyutta olmasını sağlar
                  children: [
                    Text(
                      "Yeni Kart Tanımlama", // Başlık metni
                      style:
                          Theme.of(context).textTheme.bodySmall, // Tema stili
                    ),
                    ...
                    TextFormField(
                      ...
                      controller:
                          titleCnt, // Kart başlığı metni için kontrolcü atanır
                    ),
                    ...
                    TextFormField(
                      ...
                      controller:
                          cardHolderCnt, // Kart sahibi metni için kontrolcü atanır
                    ),
                    const SizedBox(height: 10),
                    TextFormField(
                      decoration: InputDecoration(
                      ...
                      onChanged: (value) {
                        // Kart numarası girildiğinde çalışır
                        setState(() {
                          // State değiştirme
                      ...
                      children: [
                        ...
                        Expanded(
                          child: TextFormField(
                            ...
                            controller:
                                expMonthCnt, // Son kullanma ayı kontrolcüsü
                            keyboardType: TextInputType.number,
                          ),
                        ),
                        const SizedBox(width: 10),
                        Expanded(
                          child: TextFormField(
                            ...
                            controller:
                                expYearCnt, // Son kullanma yılı kontrolcüsü
                            keyboardType: TextInputType.number,
                          ),
                        ),
                      ],
                    ...
                    SwitchListTile(
                      ...
                      onChanged: (value) => setState(
                          () => remember = value), // Anahtar değiştiğinde işlem
                    ),
                    if (type.isNotEmpty) ...[
                      SizedBox(height: 20), // Boşluk ekler
                      Image.asset(
                        // Kart tipine göre görsel ekler
        ...
        actions: [
          Padding(
            ...
            child: IconButton.outlined(
                ...
                icon: const Icon(Icons.add)), // Yeni kart ekleme düğmesi
          )
        ],
      ),
      body: SafeArea(
        child: SizedBox.expand(
          child: cards.isEmpty // Kartlar listesi boşsa
              ? const Center(
                  child: Text(
                      "No Card found.")) // Ekrana "No Card found." yazısı yazdır
              : ListView.builder(
                  // Listview ile kartları görüntüle
                  itemCount: cards.length, // Kart sayısı kadar öğe
                  itemBuilder: (context, index) => AspectRatio(
                    // Kartların boyut oranını koru
                    aspectRatio: 1.586, // Belirlenen oran
                    child: Container(
                      padding:
                          const EdgeInsets.all(12), // Kenar boşlukları ayarla
                      margin:
                          const EdgeInsets.all(12), // Kenar boşlukları ayarla
                      decoration: BoxDecoration(
                        ...
                        color: Color.fromARGB(
                            255, 245, 164, 231), // Arka plan rengi
                      ),
                      child: Stack(
                        // İçeriğin üst üste binmesi için kullanılır
                        children: [
                          Positioned(
                            // Konumu ayarla
                            top: 10,
                            right: 10,
                            child: cards[index].cardNo.startsWith(
                                    "4") // VISA kartı için VISA logosunu göster
                                ? Image.asset("assets/icons/visa_card.png",
                                    height: 60)
                                : cards[index].cardNo.startsWith(
                                        "5") // Mastercard için Mastercard logosunu göster
                                    ? Image.asset(
  ...
  Widget _buildPaymentBox(String imagePath, String text, String connectedText) {
    return Container(
      ...
      child: Row(
        mainAxisAlignment:
            MainAxisAlignment.start, // Satırın başlangıcında hizalama
        children: [
          Padding(
          ...
          Text(
            ...
            style: GoogleFonts.poppins(
                fontSize: 16,
                fontWeight: FontWeight.bold), // Bağlantılı metin stili
          ),
